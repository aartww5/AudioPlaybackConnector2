# Builds a local Release x64 MSIX (unsigned) and copies release assets into ./dist/v0.5.3/
# For signed GitHub releases, push tag v0.5.3 and use the Release workflow instead.
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $repoRoot "CHANGELOG.md") -Destination (Join-Path $distRoot "CHANGELOG.md") -Force
    Copy-Item -LiteralPath (Join-Path $repoRoot "docs/releases/v$SemVer.md") -Destination (Join-Path $distRoot "RELEASE_NOTES.md") -Force
